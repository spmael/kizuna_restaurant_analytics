...
class Command(BaseCommand):
    help = _(
        "Reclassify product types for products that might have been incorrectly classified"
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--product-id",
            type=str,
            help=_("Reclassify specific product by ID"),
        )
        parser.add_argument(
            "--force",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("Starting product type reclassification...")
        )
        ...
        if product_id:
            # Reclassify specific product
            try:
                product = Product.objects.get(id=product_id)
                self._reclassify_product(product, service, dry_run, force)
            ...
            for i, product in enumerate(products, 1):
                try:
                    if self._reclassify_product(product, service, dry_run, force):
    ...
    def _reclassify_product(self, product, service, dry_run, force):
        ...
        if existing_type and not force:
            ...
            # Determine what the product type should be
            ...
            if old_product_type != new_product_type:
                ...
                if not dry_run:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"  ✓ Updated product type for {product.name}"
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            f"  [DRY RUN] Would update product type for {product.name}"
                        )
                    )
                return True
            else:
